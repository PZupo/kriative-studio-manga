      ...
      <div className="w-full max-w-5xl bg-[#1e293b] rounded-2xl border border-white/10 shadow-2xl flex flex-col h-[85vh] overflow-hidden">
        ...
        <div className="p-6 border-b border-white/10 flex justify-between items-center bg-[#161b22]">
            <div className="flex items-center gap-3">
                ...
                <div>
                    <h2 className="text-xl font-black text-white leading-none tracking-tight">{t('km_proj_title')}</h2>
                    <p className="text-xs text-slate-400 mt-1">{t('km_proj_desc')}</p>
                </div>
            </div>
        ...
        <div className="flex-1 overflow-y-auto p-6 bg-[#0f141f] custom-scroll">
            {projects.length === 0 ? (
                <div className="h-full flex flex-col items-center justify-center text-slate-600 gap-4">
                    ...
                    <p className="text-sm font-medium">{t('km_proj_empty')}</p>
                    <p className="text-xs text-slate-700">{t('km_proj_empty_hint')}</p>
                </div>
            ) : (
                    ...
                    {projects.map((p) => (
                        <div key={p.id} className="group relative bg-[#161b22] rounded-2xl border border-white/10 overflow-hidden shadow-lg hover:border-brand-teal/50 hover:shadow-brand-teal/10 transition-all flex flex-col">
                            ...
                            <div className="p-4 flex flex-col gap-4 flex-1">
                                <div>
                                    <h3 className="font-bold text-white text-sm truncate">{p.title || t('km_proj_no_title')}</h3>
                                    <p className="text-[10px] text-slate-500 uppercase tracking-wider">{p.pages.length} {t('km_page_number')}s • {p.pages[0].theme.slice(0, 30)}...</p>
                                </div>
                                ...
                                <div className="grid grid-cols-3 gap-2 mt-auto">
                                    ...
                                    <button
                                        ...
                                        title={t('km_proj_schedule_title')}
                                    >
                                        <Calendar size={14} />
                                        {t('km_proj_schedule')}
                                    </button>
